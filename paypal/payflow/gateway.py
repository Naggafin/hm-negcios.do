...
def authorize(order_number, card_number, cvv, expiry_date, amt, **kwargs):
	...
	* The hold cannot be cancelled through the PayPal API.
	...
	return _submit_payment_details(
		codes.AUTHORIZATION, order_number, card_number, cvv, expiry_date, amt, **kwargs
	)
...
def sale(order_number, card_number, cvv, expiry_date, amt, **kwargs):
	...
	"""
	return _submit_payment_details(
		codes.SALE, order_number, card_number, cvv, expiry_date, amt, **kwargs
	)


def _submit_payment_details(
	trxtype, order_number, card_number, cvv, expiry_date, amt, **kwargs
):
	"""
...
def credit(order_number, pnref, amt=None):
	...
	"""
	params = {"COMMENT1": order_number, "TRXTYPE": codes.CREDIT, "ORIGID": pnref}
	if amt:
		params["AMT"] = amt
	return _transaction(params)
...
def void(order_number, pnref):
	...
	"""
	params = {"COMMENT1": order_number, "TRXTYPE": codes.VOID, "ORIGID": pnref}
	return _transaction(params)
...
def _transaction(extra_params):
	...
	for key in constraints[trxtype]:
		if key not in extra_params:
			raise RuntimeError(
				"A %s parameter must be supplied for a %s transaction" % (key, trxtype)
			)
	...
	params = {
		...
		"USER": getattr(
			settings, "PAYPAL_PAYFLOW_USER", settings.PAYPAL_PAYFLOW_VENDOR_ID
		),
		"PARTNER": getattr(settings, "PAYPAL_PAYFLOW_PARTNER", "PayPal"),
	}
	...
	logger.info(
		"Performing %s transaction (trxtype=%s)", codes.trxtype_map[trxtype], trxtype
	)
	pairs = gateway.post(
		url, "&".join(["{}={}".format(n, v) for n, v in params.items()]), encode=False
	)
