...
class PayflowTransaction(base.ResponseModel):
	...
	pnref = models.CharField(
		_("Payflow transaction ID"), max_length=32, unique=True, null=True
	)
	ppref = models.CharField(_("Payment transaction ID"), max_length=32, null=True)
	result = models.CharField(max_length=32, null=True, blank=True)
	respmsg = models.CharField(_("Response message"), max_length=512)
	authcode = models.CharField(_("Auth code"), max_length=32, null=True, blank=True)

	# Fraud/risk params
	cvv2match = models.CharField(_("CVV2 check"), null=True, blank=True, max_length=12)
	avsaddr = models.CharField(
		_("House number check"), null=True, blank=True, max_length=1
	)
	avszip = models.CharField(
		_("Zip/Postcode check"), null=True, blank=True, max_length=1
	)
	...
	def save(self, *args, **kwargs):
		...
		self.raw_request = re.sub(
			r"ACCT=\d+(\d{4})&", "ACCT=XXXXXXXXXXXX\1&", self.raw_request
		)
