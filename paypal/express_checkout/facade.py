...
def get_paypal_url(
	basket, user=None, shipping_address=None, shipping_method=None, host=None
):
	"""
	...
	view_name = (
		"express-checkout-handle-order"
		if buyer_pays_on_paypal()
		else "express-checkout-success-response"
	)
	return_url_path = reverse(view_name, kwargs={"basket_id": basket.id})
	return_url = f"{scheme}://{host}{return_url_path}"

	cancel_url_path = reverse(
		"express-checkout-cancel-response", kwargs={"basket_id": basket.id}
	)
...
def fetch_transaction_details(token):
	...
	if not transaction.payer_id:
		...
		try:
			transaction.address_full_name = result.purchase_units[
				0
			].shipping.name.full_name
			transaction.address = json.dumps(
				result.purchase_units[0].shipping.address.dict()
			)
...
def capture_order(token):
	...
	capture_id = (
		result.id
		if transaction.is_authorization
		else result.purchase_units[0].payments.captures[0].id
	)
...
def refund_order(token):
	...
	result = PaymentProcessor().refund_order(
		transaction.capture_id, transaction.amount, transaction.currency
	)
