...
def get_paypal_url(
	...
	response_view_name = (
		"paypal-handle-order" if buyer_pays_on_paypal() else "paypal-success-response"
	)
	return_url = "%s://%s%s" % (
		scheme,
		host,
		reverse(response_view_name, kwargs={"basket_id": basket.id}),
	)
	cancel_url = "%s://%s%s" % (
		scheme,
		host,
		reverse("paypal-cancel-response", kwargs={"basket_id": basket.id}),
	)
	...
	if shipping_methods:
		update_url = "%s://%s%s" % (
			scheme,
			host,
			reverse("paypal-shipping-options", kwargs={"basket_id": basket.id}),
		)
...
def confirm_transaction(payer_id, token, amount, currency):
	...
	"""
	return do_txn(payer_id, token, amount, currency, action=_get_payment_action())


def refund_transaction(token, amount, currency, note=None):
	txn = Transaction.objects.get(token=token, method=DO_EXPRESS_CHECKOUT)
	is_partial = amount < txn.amount
	return refund_txn(
		txn.value("PAYMENTINFO_0_TRANSACTIONID"), is_partial, amount, currency
	)
...
def capture_authorization(token, note=None):
	...
	txn = Transaction.objects.get(token=token, method=DO_EXPRESS_CHECKOUT)
	return do_capture(
		txn.value("PAYMENTINFO_0_TRANSACTIONID"), txn.amount, txn.currency, note=note
	)
