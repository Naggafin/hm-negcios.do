...
def buyer_pays_on_paypal():
	return getattr(settings, "PAYPAL_BUYER_PAYS_ON_PAYPAL", False)
...
def set_txn(
	...
	return_url,
	cancel_url,
	update_url=None,  # noqa: C901 too complex
	action=SALE,
	user=None,
	user_address=None,
	...
	# level.
	params["PAYMENTREQUEST_0_ITEMAMT"] = _format_currency(basket.total_incl_tax)
	...
	if user_address:
		...
		] = user_address.country.iso_3166_1_a2
	...
	if shipping_method and shipping_address:
		...
		params[
			"PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE"
		] = shipping_address.country.iso_3166_1_a2
		...
		if params["PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE"] == "US":
			...
			if key in us_states.STATES_NORMALIZED:
				params["PAYMENTREQUEST_0_SHIPTOSTATE"] = us_states.STATES_NORMALIZED[
					key
				]
	...
	for index, method in enumerate(shipping_methods):
		is_default = index == 0
		params["L_SHIPPINGOPTIONISDEFAULT%d" % index] = (
	...
	params["PAYMENTREQUEST_0_AMT"] = _format_currency(params["PAYMENTREQUEST_0_AMT"])
	...
	params = [("cmd", "_express-checkout"), ("token", txn.token)]
...
def do_capture(txn_id, amount, currency, complete_type="Complete", note=None):
	"""
