...
class SuccessResponseView(PaymentDetailsView):
	...
	def get(self, request, *args, **kwargs):
		...
		try:
			...
			logger.warning(
				"Unable to fetch transaction details for token %s: %s", self.token, e
			)
	...
	def handle_payment(self, order_number, total, **kwargs):
		...
		self.add_payment_event(
			"Settled", confirm_txn.amount, reference=confirm_txn.correlation_id
		)
	...
	def get_shipping_address(self, basket):
		...
		return ShippingAddress(
			...
			country=Country.objects.get(
				iso_3166_1_a2=self.txn.value("PAYMENTREQUEST_0_SHIPTOCOUNTRYCODE")
			),
	...
	def get_shipping_method(self, basket, shipping_address=None, **kwargs):
		...
		if not session_method or (name and name != session_method.name):
			if name:
				method = self._get_shipping_method_by_name(
					name, basket, shipping_address
				)
			else:
				method = None
			if not method:
	...
	def render_to_response(self, methods, basket):
		...
		if methods:
			for index, method in enumerate(methods):
				...
				pairs.append(
					("L_SHIPPINGOPTIONLABEL%d" % index, str(method.description))
				)
				...
				pairs.append(
					("L_SHIPPINGOPTIONISDEFAULT%d" % index, 1 if index == 0 else 0)
				)
		else:
		...
		logger.debug(
			"Basket #%s - returning postage costs payload = '%s'", basket.id, payload
		)
